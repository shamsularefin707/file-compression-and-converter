          ...
          <div className="space-y-3 font-sans">
            {queue.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="p-4 rounded-2xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800/80 shadow-sm flex flex-col gap-1.5 transition-all"
                >
                  ...
                  {/* Quality Report sub-component */}
                  {item.report && (
                    <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-900">
                      <div className="p-3.5 rounded-xl bg-slate-50/50 dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800/50">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-center">
                          <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-100 dark:border-slate-900">
                            <p className="text-[10px] text-slate-400 font-medium">Pages</p>
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{item.report.pages}</p>
                          </div>
                          <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-100 dark:border-slate-900">
                            <p className="text-[10px] text-slate-400 font-medium">Headings</p>
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{item.report.headings}</p>
                          </div>
                          <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-100 dark:border-slate-900">
                            <p className="text-[10px] text-slate-400 font-medium">Tables</p>
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{item.report.tables}</p>
                          </div>
                          <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-100 dark:border-slate-900">
                            <p className="text-[10px] text-slate-400 font-medium">Links</p>
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{item.report.links}</p>
                          </div>
                          <div className="bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-100 dark:border-slate-900 col-span-2 sm:col-span-1">
                            <p className="text-[10px] text-slate-400 font-medium">Text Blocks</p>
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{item.report.textBlocks}</p>
                          </div>
                        </div>
                        
                        {item.report.warnings && item.report.warnings.length > 0 ? (
                          <div className="mt-2.5 space-y-1">
                            {item.report.warnings.map((warn: string, wIdx: number) => (
                              <div key={wIdx} className="flex items-start gap-1.5 text-xxs text-amber-600 dark:text-amber-400 font-medium bg-amber-500/5 px-2 py-1 rounded-md border border-amber-500/10">
                          ...
                          <div className="mt-2.5 flex items-center gap-1.5 text-xxs text-emerald-600 dark:text-emerald-400 font-medium bg-emerald-500/5 px-2 py-1 rounded-md border border-emerald-500/10">
                            <CheckCircle2 className="w-3 h-3 shrink-0" />
                            <span>Semantic reconstruction completed with no layout structure losses.</span>
                          </div>
                        )}
                      </div>
                    </div>
